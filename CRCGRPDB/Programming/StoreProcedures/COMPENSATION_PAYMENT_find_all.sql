Create Procedure EMPLOYEES.COMPENSATION_PAYMENT_find_all
(
...
Begin
	...
	--EXECUTE EMPLOYEES.COMPENSATION_PAYMENT_find_all '<Record> <employee_id>1</employee_id> </Record>',0,0,'',NULL;
	...
	Begin
		...
		Begin
			...
			DB.interbank_account			[interbank_account],
			DB.bank_id			[bank_id],
			DB.account_type_id			[account_type_id],
			DB.currency_id			[currency_id],
			...
			Set @sqlJoin = '
			From EMPLOYEES.COMPENSATION_PAYMENT [DB]'
		End;
		--WHERE SQL
		Begin
